          ...
          {!open && (
            ...
            >
              ...
              <div className="absolute inset-0 animate-breathe rounded-full opacity-40"
                style={{ background: "radial-gradient(circle, rgba(37,99,235,0.5), rgba(20,184,166,0.3), transparent 70%)", filter: "blur(8px)" }} />
              <div className="absolute -inset-1 animate-[breathe_3s_ease-in-out_infinite_0.5s] rounded-full opacity-20"
                style={{ background: "radial-gradient(circle, rgba(37,99,235,0.4), transparent 70%)", filter: "blur(12px)" }} />
              ...
              <div className="relative h-14 w-14 overflow-hidden rounded-full shadow-glow transition-transform duration-200 group-hover:scale-110"
                style={{
                  background: "linear-gradient(135deg, rgba(255,255,255,0.9) 0%, rgba(186,230,253,0.6) 25%, rgba(37,99,235,0.5) 60%, rgba(20,184,166,0.7) 100%)",
                  boxShadow: "0 0 0 1px rgba(255,255,255,0.5) inset, 0 0 24px rgba(37,99,235,0.3), 0 8px 32px rgba(0,0,0,0.12)",
                }}>
                {/* Inner crystal shine */}
          ...
          <>
            ...
            <motion.div
              ...
              style={{
                ...
                boxShadow: "0 1px 0 rgba(255,255,255,0.95) inset, 0 8px 32px rgba(0,0,0,0.12), 0 32px 64px rgba(0,0,0,0.10), 0 0 0 1px rgba(37,99,235,0.08)",
              }}
            >
              {/* Header */}
              <div className="flex items-center justify-between border-b border-slate-100/60 px-5 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="h-8 w-8 overflow-hidden rounded-full"
                    style={{
                      background: "linear-gradient(135deg, rgba(255,255,255,0.9), rgba(186,230,253,0.6) 30%, rgba(37,99,235,0.6) 70%, rgba(20,184,166,0.8))",
                      boxShadow: "0 0 12px rgba(37,99,235,0.25), 0 1px 0 rgba(255,255,255,0.5) inset",
                    }}
                  >
                ...
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-100/80 text-xs text-ink-muted transition-colors hover:bg-slate-200/80"
              ...
              <div className="px-5 py-4">
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200/60 bg-white/60 px-4 py-2.5 text-sm text-ink placeholder:text-ink-subtle outline-none backdrop-blur-sm transition-all focus:border-accent/40 focus:bg-white/80 focus:ring-2 focus:ring-accent/10"
                  />
                  <button
                    type="button"
                    className="absolute right-2 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-lg bg-accent text-white shadow-glow transition-all hover:bg-blue-500 active:scale-95 disabled:opacity-40"
              ...
              <div className="border-t border-slate-100/60 px-3 py-3">
                ...
                <div className="flex flex-col gap-0.5">
                  {AI_COMMANDS.map((cmd) => (
                    ...
                    >
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-blue-50 to-sky-50 text-sm font-bold text-accent">
                        {cmd.icon}
                      </span>
                      <span>
