...
export function ThemePanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <>
      {/* Backdrop tutup-saat-klik (transparan, di bawah panel) */}
      ...
      <motion.div
        ...
        className="absolute right-0 top-full z-50 mt-2 w-80 overflow-hidden rounded-2xl border border-glass-border bg-glass-strong shadow-float backdrop-blur-xl"
        role="dialog"
      ...
      >
        ...
        <div className="flex items-center gap-1 border-b border-glass-border-subtle p-2">
          ...
          <button
            ...
            className="grid h-7 w-7 shrink-0 place-items-center rounded-full text-ink-muted transition-colors hover:bg-black/5 hover:text-ink"
          >
            ×
          </button>
        </div>
        ...
        {tab === "tema" ? (
          <div className="scrollbar-thin max-h-[26rem] overflow-y-auto p-3">
            ...
            <div className="mb-3 flex flex-wrap gap-1.5">
              {PERSONA_RECOMMENDATIONS.map((p) => (
                <button
                  ...
                  onClick={() => setActiveTheme(p.themeId)}
                  className="rounded-full border border-glass-border-strong bg-glass px-2.5 py-1 text-[11px] font-medium text-ink-muted transition-colors hover:border-accent/40 hover:bg-accent-soft hover:text-accent"
                >
                  {p.persona}
                </button>
              ))}
              ...
              {themes.map((t) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="truncate px-2 pb-1.5 pt-1 text-[11px] font-medium text-ink">
                      {t.name}
                    </p>
                  </button>
                );
            ...
            {/* AI Theme Generator */}
            <p className="mb-1.5 mt-4 text-[10px] font-semibold uppercase tracking-wide text-ink-subtle">
              AI Theme Generator
            </p>
            <div className="flex gap-1.5">
              <input
                ...
                className="min-w-0 flex-1 rounded-lg border border-glass-border-strong bg-glass px-2.5 py-1.5 text-xs text-ink outline-none transition-all placeholder:text-ink-subtle focus:border-accent/50 focus:ring-2 focus:ring-accent/15"
              />
              <button
                type="button"
                onClick={onGenerate}
                disabled={!prompt.trim()}
                className="kv-cta rounded-lg px-3 py-1.5 text-xs font-semibold transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
              >
                Buat
              </button>
            </div>
            ...
            {COMFORT_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-3 rounded-xl p-2.5 text-left transition-colors hover:bg-black/5"
                >
                  <span className="min-w-0 flex-1">
                    <span className="block text-xs font-semibold text-ink">
                      {opt.label}
                    </span>
                    <span className="block text-[10px] leading-relaxed text-ink-muted">
                      {opt.desc}
                    </span>
                  </span>
                  <span
